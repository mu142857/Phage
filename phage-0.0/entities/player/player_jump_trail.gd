...
# 残影用主角当前帧;所有可见的特效层(盾壳/蓝水晶/火光…)当前帧作为子精灵一起复制。
# =============================================================================
extends Node2D
...
func _spawn_ghost_custom(ghost_tint: Color, ghost_alpha: float, lifetime: float) -> void:
	...
	# 可见的特效层(盾壳/蓝水晶/火光…)当前帧作为子精灵叠上,跟着父级一起淡出。
	for child in _sprite.get_children():
		var overlay := child as AnimatedSprite2D
		if overlay == null or not overlay.visible:
			continue
		var overlay_tex := _overlay_frame_texture(overlay)
		if overlay_tex == null:
			continue
		var og := Sprite2D.new()
		og.texture = overlay_tex
		og.offset = overlay.offset
...
# overlay 当前帧贴图(没有对应动画时返回 null),同样做防渗色处理。
func _overlay_frame_texture(overlay: AnimatedSprite2D) -> Texture2D:
	var frames := overlay.sprite_frames
	if frames == null or not frames.has_animation(overlay.animation):
		return null
	var tex := frames.get_frame_texture(overlay.animation, overlay.frame)
	if tex is AtlasTexture:
		var clipped := AtlasTexture.new()
		clipped.atlas = (tex as AtlasTexture).atlas
		clipped.region = (tex as AtlasTexture).region
