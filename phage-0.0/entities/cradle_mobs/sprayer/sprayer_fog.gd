...
# setup(start_pos, dir: Vector2, speed)
extends Area2D
...
func _ready() -> void:
	...
	# 每颗雾大小微随机,一团雾才不整齐
	if _square != null:
		var k := randf_range(0.8, 1.3)
		_square.scale = Vector2(k, k)


func setup(start_pos: Vector2, dir: Vector2, speed: float) -> void:
	global_position = start_pos
	_dir = dir.normalized() if dir != Vector2.ZERO else Vector2.LEFT
	_speed = speed
	_active = true
...
func _physics_process(delta: float) -> void:
	if not _active:
		return
	global_position += _dir * _speed * delta   # 散掉时 _speed 已被压低,只是慢慢飘
	if _done:
		return
	_elapsed += delta
	_spawn_trail(delta)
...
func _check_touch() -> void:
	for body in get_overlapping_bodies():
		...
		if body.is_in_group("earthworm"):
			_dissipate(0.4, 0.25)   # 被蚯蚓挡住:放慢、慢慢散掉(它的碰撞箱随动画变化很大,别硬邦邦地瞬间消失)
			return
...
# 散掉:缩小+淡出,不凭空消失。duration=淡多久;drift=淡的时候还按几成速度往前飘
